...
int main(int argc, char *argv[]) {
    if (argc > 1) {
        ...
        if (strcmp(command, "predict_power_grid_load") == 0) {
            ...
            const char *model_path = "assets/grid_predictor.tflite";

            MainService *main_service = main_service_new(auth_token, model_path);
            printf("[*] Getting power data for the last 24 hours...\n");

            float result = predict_power_grid_load(main_service);
            if (result != -1) {
